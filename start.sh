...
#
# Tests live in tests/ and are run separately via tests/run-tests.sh.
...
Tests are NOT run by start.sh anymore. Use:
  ./tests/run-tests.sh                # backend pytest + frontend node tests
  ./tests/run-tests.sh --e2e          # also run Playwright browser tests
